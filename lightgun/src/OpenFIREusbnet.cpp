...
// ====================================================================
// BUGFIX PER WINDOWS: INTERCETTATORE DEDICATO NCM
// ====================================================================
// La libreria Adafruit TinyUSB originale ha un bug nell'inizializzazione NCM:
// - Non gestisce l'interrogazione MAC Address (0x81)
// - Risponde a NTB Parameters (0x80) con un wNdbOutDivisor di 1 (violando le spec Windows)
// Questo interceptor si posiziona prima del driver nativo aggirando i due bug, 
// per impedire a usbncm.sys di andare in arresto anomalo (Codice 10).
extern "C" {
    extern void netd_init(void);
    extern void netd_reset(uint8_t rhport);
    extern uint16_t netd_open(uint8_t rhport, tusb_desc_interface_t const *itf_desc, uint16_t max_len);
    extern bool netd_control_xfer_cb(uint8_t rhport, uint8_t stage, tusb_control_request_t const *request);
    extern bool netd_xfer_cb(uint8_t rhport, uint8_t ep_addr, xfer_result_t result, uint32_t xferred_bytes);

    void dummy_netd_init(void) {
        // Ignora: l'array in BSS è già a zero. Serve solo a by-passare il native init
    }

    alignas(4) static uint8_t ntb_parameters_ram[28] = {
        0x1C, 0x00,             // wLength
        0x01, 0x00,             // bmNtbFormatsSupported
        0x80, 0x0C, 0x00, 0x00, // dwNtbInMaxSize = 3200
        0x04, 0x00,             // wNdpInDivisor = 4
        0x00, 0x00,             // wNdpInPayloadRemainder
        0x04, 0x00,             // wNdpInAlignment
        0x00, 0x00,             // wReserved
        0x80, 0x0C, 0x00, 0x00, // dwNtbOutMaxSize = 3200
        0x04, 0x00,             // wNdpOutDivisor = 4 (FONDAMENTALE per Windows)
    ...
    bool custom_netd_control_xfer_cb(uint8_t rhport, uint8_t stage, tusb_control_request_t const *request) {
        if (request->bmRequestType_bit.type == TUSB_REQ_TYPE_CLASS && request->bmRequestType_bit.recipient == TUSB_REQ_RCPT_INTERFACE) {
            // NTB Parameters
            if (request->bRequest == 0x80) {
                if (stage == CONTROL_STAGE_SETUP) {
                    return tud_control_xfer(rhport, request, ntb_parameters_ram, sizeof(ntb_parameters_ram));
                }
                return true;
            }
            // MAC Address
        ...
        if (request->bmRequestType_bit.type == TUSB_REQ_TYPE_STANDARD && request->bRequest == TUSB_REQ_SET_INTERFACE) {
            if (stage == CONTROL_STAGE_SETUP) {
                OF_NcmDataInterfaceReady = (request->wValue == 1);
            }
        }
    ...
    usbd_class_driver_t const* usbd_app_driver_get_cb(uint8_t* driver_count) {
        if (OpenFIREUsbNetActive()) {
            if (driver_count) *driver_count = 1;
            return &ncm_app_driver;
        }
        if (driver_count) *driver_count = 0;
        return NULL;
    }
}
// ====================================================================
